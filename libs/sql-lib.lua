...
function M:getMusicRowBySearch(index, ascending, search, limit)
	...
	if (index > 1) then
		stmt =
			database:prepare(
			sFormat(
				[[ SELECT * FROM `music` WHERE albumSearch %s %s %s ORDER BY sortTitle %s LIMIT 1 OFFSET %d; ]],
				likeQuery,
				artistQuery,
				titleQuery,
				orderType,
		...
		stmt =
			database:prepare(
			sFormat(
				[[ SELECT * FROM `music` WHERE albumSearch %s %s %s ORDER BY sortTitle %s LIMIT 1; ]],
				likeQuery,
				artistQuery,
				titleQuery,
				orderType
...
function M:getMusicRowsBySearch(index, ascending, orderBy, search, limit)
	...
	if (index > 1) then
		stmt =
			database:prepare(
			sFormat(
				[[ SELECT * FROM `music` WHERE albumSearch %s %s %s ORDER BY %s %s LIMIT %d OFFSET %d; ]],
				likeQuery,
				artistQuery,
				titleQuery,
				orderBy,
		...
		stmt =
			database:prepare(
			sFormat(
				[[ SELECT * FROM `music` WHERE albumSearch %s %s %s ORDER BY %s %s LIMIT %d; ]],
				likeQuery,
				artistQuery,
				titleQuery,
				orderBy,
