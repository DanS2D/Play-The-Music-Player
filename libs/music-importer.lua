					...
					if (not isDir and isMusicFile(file)) then
						if (fileUtils:fileSize(fullPath) > 0) then
							...
							local musicData = {
								fileName = file,
								filePath = path,
								title = tags.title:len() > 0 and tags.title:stripLeadingSpaces() or file,
								artist = tags.artist:stripLeadingSpaces(),
								album = tags.album:stripLeadingSpaces(),
								genre = tags.genre:stripLeadingSpaces(),
								comment = tags.comment:stripLeadingSpaces(),
								year = tags.year,
