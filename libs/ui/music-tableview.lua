...
local function createTableView(tableView, options)
	...
	function view:_renderCategory(row, reloadDataInvocation)
		...
		local function initNewCategory()
			...
			if isGraphicsV1 then
				...
				self._currentCategory.anchorX = 0.5
				self._currentCategory.anchorY = 0.5
			end
			local catX = self.x
			if isGraphicsV1 then
		...
		for k, v in pairs(self._rows) do
			...
			if type(currentRow._view) ~= "table" then
				...
				local isRowWithinBounds =
					(currentRow.y + self.y) + currentRow._height > upperLimit and
					(currentRow.y + self.y) - currentRow._height < lowerLimit
			...
			if self._numCategories > 0 then
				...
				if type(currentRow._view) == "table" then
					...
					elseif
						currentRow.isCategory and currentRow._top >= 0 and self._currentCategory and
							currentRow.index == self._currentCategory.index
					 then
			...
			if "table" == type(currentRow._view) then
				-- Is this row within the visible bounds of our view?
				local isRowWithinBounds =
					(currentRow.y + self.y) + currentRow._height * 2 > upperLimit and
					(currentRow.y + self.y) - currentRow._height * 2 < lowerLimit
		...
		if isRowWithinBounds then
			...
			if type(currentRow._view) ~= "table" then
				...
				if not self._noLines and not currentRow.isCategory then
					...
					rowLine =
						display.newRect(currentRow._view, leftPadding, rowCell.y, currentRow._width - rightPadding - leftPadding, 1)
					if isGraphicsV1 then
						rowLine:setReferencePoint(display.CenterReferencePoint)
				...
				if isGraphicsV1 then
					...
					currentRow._view.anchorX = 0.5
					currentRow._view.anchorY = 0.5
				end
				...
				function currentRow._view:setRowColor(options)
					if "table" == type(options) then
						...
						print(
							"WARNING: Function 'row:setRowColor()' - options table with default/over tables expected, got",
							type(options)
						)
					end
				end
	...
	function view:_insertRow(options, reRender)
		...
		local rowColor = options.rowColor or {0, 0, 0}
		-- colours.rowColor
		local lineColor = options.lineColor or {1, 1, 1} --colours.lineColor
		...
		if table.maxn(self._rows) <= 1 then
			...
			if (self._rows[table.maxn(self._rows) - 1].y) then
				self._rows[table.maxn(self._rows)].y =
					(self._rows[table.maxn(self._rows) - 1].y + (self._rows[table.maxn(self._rows) - 1]._height * 0.5)) +
	...
	function view:_deleteRows(rowIndexesTable, animationOptions)
		if "table" ~= type(rowIndexesTable) then
			print(
				"WARNING: Function 'deleteRows()' accepts a table of row indexes as a parameter, for example 'tableView:deleteRows( { 1, 3, 5 } )'"
			)
			return
		else
			if #rowIndexesTable < 1 then
				print(
					"WARNING: Function 'deleteRows()' accepts a table with at least one row index as a parameter, for example 'tableView:deleteRows( { 1, 3, 5 } )'"
				)
				return
			end
		end
						...
						if nil ~= self._rows[i]._view and "table" == type(self._rows[i]._view) then
							...
							if self._rows[i].isCategory then
								if nil ~= self._rows[i - 1] then
									transition.to(
										self._rows[i]._view,
										{y = self._rows[i]._view.y - heightToBeDeleted, time = slideUpAnimationTime, transition = easing.outQuad}
									)
		...
		for i = 1, #rowsToBeRemoved do
			...
			if i == #rowsToBeRemoved then
				transition.to(
					row._view,
					{
						x = -(row._view.contentWidth * 0.5),
						time = slideLeftAnimationTime,
						transition = easing.inQuad,
						onComplete = function()
							removeRows(rowsToBeRemoved, deletedContentHeight)
	...
	function view:_scrollToIndex(...)
		...
		-- Transition the view
		self._transitionToIndex =
			transition.to(
			self,
			{y = newPosition, time = scrollTime, transition = easing.outQuad, onComplete = executeOnComplete}
		)
	...
	function view:_scrollToY(options)
		...
		self._transitionToIndex =
			transition.to(
			self,
			{y = newY, time = transitionTime, transition = easing.inOutQuad, onComplete = onTransitionComplete}
		)
	end
	...
	function view:_reloadData()
		...
		for i, row in pairs(self._rows) do
			local isRowWithinBounds =
				(row.y + self.y) + row._height > upperLimit and (row.y + self.y) - row._height * 2 < lowerLimit
	...
	function tableView:addListeners()
		viewBackground:addEventListener("touch")
		view:addEventListener("touch")
	end

	function tableView:removeListeners()
		viewBackground:removeEventListener("touch")
		view:removeEventListener("touch")
	end
